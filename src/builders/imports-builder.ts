...
export class ImportsBuilderImpl implements ImportsBuilder {
  ...
  named(name: string, alias?: string): this {
    ...
    const existingImport = this.namedImports.find(imp => imp.name === name && imp.alias === alias);
    if (!existingImport) {
  ...
  generate(): string {
    ...
    const includeNamespace = this.namespaceImport && (this.options.includeUnused || this.namespaceImport.used);
